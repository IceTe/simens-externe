...
namespace Chirurgie\Controller;

use Chirurgie\Form\AdmissionForm;
use Chirurgie\Form\ConsultationForm;
use Chirurgie\Form\PatientForm;
use Chirurgie\View\Helper\DateHelper;
use Chirurgie\View\Helper\DemandeExamenBioPdf;
use Chirurgie\View\Helper\DemandeExamenFonctionnelPdf;
use Chirurgie\View\Helper\DemandeExamenMorphoPdf;
//use Chirurgie\View\Helper\DemandeExamenPdf;
use Chirurgie\View\Helper\DocumentPdf;
use Chirurgie\View\Helper\HospitalisationPdf;
use Chirurgie\View\Helper\OrdonnancePdf;
use Chirurgie\View\Helper\RendezVousPdf;
use Chirurgie\View\Helper\TraitementChirurgicalPdf;
use Chirurgie\View\Helper\TraitementInstrumentalPdf;
use Chirurgie\View\Helper\TransfertPdf;
use Zend\Json\Json;
...
class ChirurgieController extends AbstractActionController {
	...
	public function getResultatExamensComplentairesTable(){
		if(!$this->ResultatExamensComplementaires){
			...
			$this->ResultatExamensComplementaires = $sm->get('Chirurgie\Model\ResultatExamensComplementairesTable');
		}
	...
	public function getOrganeTable() {
	    if (! $this->organeTable) {
	        $sm = $this->getServiceLocator ();
	        $this->organeTable = $sm->get ( 'Chirurgie\Model\OrganeTable' );
	    }
	...
	public function getOrdonConsommableTable() {
		if (! $this->ordonConsommableTable) {
			$sm = $this->getServiceLocator ();
			$this->ordonConsommableTable = $sm->get ( 'Chirurgie\Model\OrdonConsommableTable' );
		}
	...
	public function getDemandeActe() {
		if (! $this->demandeActeTable) {
			$sm = $this->getServiceLocator ();
			$this->demandeActeTable = $sm->get ( 'Chirurgie\Model\DemandeActeTable' );
		}
	...
	public function getRvPatientConsTable() {
		if (! $this->rvPatientConsTable) {
			$sm = $this->getServiceLocator ();
			$this->rvPatientConsTable = $sm->get ( 'Chirurgie\Model\RvPatientConsTable' );
		}
	...
	public function getResultatVpa() {
		if (! $this->resultatVpaTable) {
			$sm = $this->getServiceLocator ();
			$this->resultatVpaTable = $sm->get ( 'Chirurgie\Model\ResultatVisitePreanesthesiqueTable' );
		}
	...
	public function getDemandeVisitePreanesthesiqueTable() {
		if (! $this->demandeVisitePreanesthesiqueTable) {
			$sm = $this->getServiceLocator ();
			$this->demandeVisitePreanesthesiqueTable = $sm->get ( 'Chirurgie\Model\DemandeVisitePreanesthesiqueTable' );
		}
	...
	public function getOrdonnanceTable() {
		if (! $this->ordonnanceTable) {
			$sm = $this->getServiceLocator ();
			$this->ordonnanceTable = $sm->get ( 'Chirurgie\Model\OrdonnanceTable' );
		}
	...
	public function getDiagnosticsTable() {
		if (! $this->diagnosticsTable) {
			$sm = $this->getServiceLocator ();
			$this->diagnosticsTable = $sm->get ( 'Chirurgie\Model\DiagnosticsTable' );
		}
	...
	public function getNotesExamensMorphologiquesTable() {
		if (! $this->notesExamensMorphologiquesTable) {
			$sm = $this->getServiceLocator ();
			$this->notesExamensMorphologiquesTable = $sm->get ( 'Chirurgie\Model\NotesExamensMorphologiquesTable' );
		}
	...
	public function getDonneesExamensPhysiquesTable() {
		if (! $this->donneesExamensPhysiquesTable) {
			$sm = $this->getServiceLocator ();
			$this->donneesExamensPhysiquesTable = $sm->get ( 'Chirurgie\Model\DonneesExamensPhysiquesTable' );
		}
	...
	public function getAntecedantPersonnelTable() {
		if (! $this->antecedantPersonnelTable) {
			$sm = $this->getServiceLocator ();
			$this->antecedantPersonnelTable = $sm->get ( 'Chirurgie\Model\AntecedentPersonnelTable' );
		}
	...
	public function getAntecedantsFamiliauxTable() {
		if (! $this->antecedantsFamiliauxTable) {
			$sm = $this->getServiceLocator ();
			$this->antecedantsFamiliauxTable = $sm->get ( 'Chirurgie\Model\AntecedentsFamiliauxTable' );
		}
	...
	public function getTransfererPatientServiceTable() {
		if (! $this->transfererPatientServiceTable) {
			$sm = $this->getServiceLocator ();
			$this->transfererPatientServiceTable = $sm->get ( 'Chirurgie\Model\TransfererPatientServiceTable' );
		}
	...
	public function getMotifAdmissionTable() {
		if (! $this->motifAdmissionTable) {
			$sm = $this->getServiceLocator ();
			$this->motifAdmissionTable = $sm->get ( 'Chirurgie\Model\MotifAdmissionTable' );
		}
	...
	public function getDemandeHospitalisationTable() {
		if (! $this->demandeHospitalisationTable) {
			$sm = $this->getServiceLocator ();
			$this->demandeHospitalisationTable = $sm->get ( 'Chirurgie\Model\DemandehospitalisationTable' );
		}
	...
	public function demandeExamensTable() {
		if (! $this->demandeExamensTable) {
			$sm = $this->getServiceLocator ();
			$this->demandeExamensTable = $sm->get ( 'Chirurgie\Model\DemandeTable' );
		}
	...
	public function getConsultationTable() {
		if (! $this->consultationTable) {
			$sm = $this->getServiceLocator ();
			$this->consultationTable = $sm->get ( 'Chirurgie\Model\ConsultationTable' );
		}
	...
	public function getAdmissionTable() {
		if (! $this->admissionTable) {
			$sm = $this->getServiceLocator ();
			$this->admissionTable = $sm->get ( 'Chirurgie\Model\AdmissionTable' );
		}
	...
	public function getServiceTable() {
		if (! $this->serviceTable) {
			$sm = $this->getServiceLocator ();
			$this->serviceTable = $sm->get ( 'Chirurgie\Model\ServiceTable' );
		}
	...
	public function getTarifConsultationTable() {
		if (! $this->tarifConsultationTable) {
			$sm = $this->getServiceLocator ();
			$this->tarifConsultationTable = $sm->get ( 'Chirurgie\Model\TarifConsultationTable' );
		}
	...
	public function getPatientTable() {
		if (! $this->patientTable) {
			$sm = $this->getServiceLocator ();
			$this->patientTable = $sm->get ( 'Chirurgie\Model\PatientTable' );
		}
	...
	public function majComplementConsultationAction() {
	    ...
	    $this->layout ()->setTemplate ( 'layout/chirurgie' );
	   ...
	   $tab= array();$h=1;
	    foreach ($listeResultatsExamensBiologiques as $exam_fonc){
	    	...
	    	if($exam_fonc["libelleExamen"]== "h??mogramme sanguin"){
	    	;
	    		$data["hemogramme_sanguin"] = $exam_fonc["noteResultat"];
	    	}
	    	 
	    	if($exam_fonc["libelleExamen"]== "bilan h??patique"){
	    		$data["bilan_hepatique"] = $exam_fonc["noteResultat"];
	    	}
	    	 
	    	if($exam_fonc["libelleExamen"]== "bilan r??nal"){
	    		$data["bilan_renal"] = $exam_fonc["noteResultat"];
	    	}
	    	
	    	if($exam_fonc["libelleExamen"]== "Bilan h??mostase"){
...
// 	
	...
	public function listeRendezVousAction() {
		...
		$layout = $this->layout ();
		$layout->setTemplate ( 'layout/chirurgie' );
		...
		$this->getConsultationTable ()->validerConsultation ( $valide );
			...
			return $this->redirect ()->toRoute ( 'chirurgie', array (
	...
	public function updateComplementConsultationAction(){
		...
		// **********-- Ajout des demandes  d'examens complementaires  ?? faire pour le patient --********
		// **********-- Ajout des demandes  d'examens complementaires  ?? faire pour le patient --********
		// **********-- Ajout des demandes  d'examens complementaires  ?? faire pour le patient --********
		...
		//Ajouter les ant??c??dents chirurgicaux
	...
	//CONSULTATION DU MEDECIN DE LA CHirurgie GENERALE
...
// 		$data['irm'] = $examen_morphologique['irm'];
	...
	//Admission d' un patient en Chirurgie
...
// 		$form->setData ( $formData );
		...
		//$admissionTable= new AdmissionTable($tableGateway);
		//$id_admission1= $admissionTable->addAdmission($donnees);
		...
		//var_dump($id_admission);exit();
		//$this->getAdmissionTable ()->addConsultationChirurgieGenerale($id_cons);
		...
		return $this->redirect()->toRoute('chirurgie', array('action' =>'liste-patients-admis'));
...
// 		}
	...
	public function lecteurMp3Action($ListeDesSons){
	    ...
	    $html .='
				<form id="my_form" method="post" action="/simens/public/chirurgie/ajouter-mp3" enctype="multipart/form-data">
	...
	public function lecteurMp3InstrumentalAction($ListeDesSons){
	    ...
	    $html .='
				<form id="my_form2" method="post" action="/simens/public/chirurgie/ajouter-mp3" enctype="multipart/form-data">
	...
	public function lecteurVideoAction($ListeDesVideos){
		...
		<form id="my_form_video" method="post" action="/simens/public/chirurgie/ajouter-video" enctype="multipart/form-data">
